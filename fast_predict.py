...
data_ko2en = KOEN(batch_size=batch_size, mode='test', src_lang='en', tgt_lang='ko', preload_voca=True)
...
ret = []
beam_size = 12
for i in range(5): #data_ko2en.num_batch * beam_size):
    t_str = time.time()

    beam_batch_size = batch_size // beam_size # 64
    predictions = beam_run(sess, data_ko2en.source[i*beam_batch_size:(i+1)*beam_batch_size], beam_batch_size, beam_size)
    clean(sess)
    ret.extend(data_ko2en.print_index(predictions, sysout=False))

    t_elp = time.time() - t_str
    t_avg = 0.9*t_avg + 0.1*t_elp
    t_rem = t_avg * (data_ko2en.num_batch * beam_size-i-1)
    print('[%d: %.2fs, %dm]' % (i, t_elp, t_rem//60), t_avg, end='\r')
print()

with open('./%s.ko.test_result' % model_name, 'w') as f:
